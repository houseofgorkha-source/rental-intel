                                                    ...
                                                    **[Verified Fact]**
                                                    ...
                                                    app/
                                                      ...
                                                      account/          Authenticated account area (see §26): layout.tsx (auth gate + nav),
                                                                        overview, properties/, properties/[slug]/edit, reviews/, wishlist/, verifications/, messages/, profile/
                                                    ...
                                                    components/
                                                      ...
                                                      home/             ListYourPropertySection (the homepage's role entry points, §26), BrokerInterestSurvey.tsx (homepage
                                                                        yes/no poll on whether to build a broker feature, not the feature itself — see §7 migration 20)
                                                    ...
                                                    **`/account` is intentionally thin — amended twice.** When this was first written, five routes were the deliberate ceiling, with messaging and wishlist UI named explicitly as things to stay excluded. Both have since been built, as a considered exception rather than scope creep: **`/account` now has seven routes** — overview (counts only), properties, reviews (reuses `ReviewCard`), **wishlist** (added — see below), verifications, messages, profile (`display_name`, the sole editable `profiles` column) — behind the same one `layout.tsx` chrome. **Every one of these queries is permitted by pre-existing RLS or a narrowly-added one — no blanket widening.**

                                                    Messaging is real but stays deliberately minimal: `property_contacts`/`property_messages` (migration 12) let a contributor who chose "Message here" receive one message and send one reply per sender (migration 14 widened this from strictly one-way) — no thread view, no push notifications, only an unread-count badge on the Account menu (computed in `app/layout.tsx`, rendered in `AccountMenu.tsx`). Wishlist now has its full list view too: `/account/wishlist` reuses `PropertyList` (§20 — no bespoke card), backed by `getWishlistedProperties()` in `lib/property-discovery.ts`, which shares its property-enrichment logic with `getDiscoveryProperties()` via an extracted `enrichProperties()` helper rather than a second implementation. No new RLS was needed — `wishlists` has had a "read your own" policy since the initial schema; this is the first UI to use it.
